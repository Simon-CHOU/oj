...
public interface IPlug {
	 ...
	 * @param logDir     日志存放目录(绝对路径);在PerformPlug中,该参数为测试用例所在目录
	 * @return 执行结果(多个字段)：studentInfor(学生信息)、
	 *         value(执行结果-某类错误小于5个错一个扣一分,大于5个则该检查项设置的分数全部扣除;)、
	 *         message(执行成功返回"OK",反之返回错误描述)、code(执行成功返回0,反之返回-1)
	 *         特别的,在PerformPlug中,value字段存储作业执行后的结果描述,如Accepted;
	 *         message字段存储作业执行消耗的最大时间和最大内存,以;分隔,如"1000;65535"
	 */
	 ...
	 * @return 执行结果集合(多个字段)：studentInfor(学生信息)、
	 *         value(执行结果-某类错误小于5个错一个扣一分,大于5个则该检查项设置的分数全部扣除)、
	 *         message(执行成功返回"OK",反之返回错误描述)、code(执行成功返回0,反之返回-1)
	 */
