...
public interface IGccPlug {
	 ...
	 * @return 若执行成功,则返回执行结果(包括学号和执行结果信息),反之返回null
	 * 			执行结果信息分七类:Accepted(通过)、Wrong Answer(不通过)、Compile Error(编译错误)、
	 * 			Time Limit Exceeded(超时)、Memory Limit Exceeded(超出限定内存)、
	 ...
	 * @param log_dir     日志存放目录(绝对路径)
	 * @return  返回批量执行结果(包括学号和执行结果信息)
	 *   		执行结果信息分七类:Accept(通过)、Wrong Answer(不通过)、Compile Error(编译错误)、
	 * 			Time Limit Exceeded(超时)、Memory Limit Exceeded(超出限定内存)、
	 * 			Presentation Error(输出格式错误)、Runtime Error(运行错误)
	 * @throws FileNotFoundException     If src_dir or log_dir not found.
	 * @throws FileNotDirectoryException If src_dir or log_dir is not directory.
