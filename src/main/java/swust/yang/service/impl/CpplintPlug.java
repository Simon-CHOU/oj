...
public class CpplintPlug implements IPlug {
	...
	public ResultMsg singleExecute(String configInfo, String toolPath, 
		...
		// 执行作业
		ExecMessage ret	= ExecUtil.exec(executeCommand);
	    if(ret.getError() != null) {
	    	System.err.println("作业执行失败！错误描述是：" + ret.getError());
	    	System.err.println("错误原因可能是：");
		...
		msg.setStudentInfo(studentInfo);
	...
	public List<ResultMsg> batchExecute(String configInfo, String toolPath, 
		...
		for (String item : fileList) {
			...
			if (ret != null) {
				...
				ResultMsg msg = new ResultMsg();
				msg.setStudentInfo(item.substring(0, item.lastIndexOf('.')));
				msg.setValue("0");
				msg.setCode(-1);
	...
	public String getHtml(String preSetting) {
		...
		if (preSetting != null) {
			...
			html = "<div id = \"cpplint\" >\r\n" + 
					"		总分\r\n" + 
					"		<input type=\"number\" name=\"totalScore\" value=" + '"'+ configObj.getTotalScore().toString().substring(0, configObj.getTotalScore().toString().indexOf('.')) + '"' + " min=\"1\" max=\"100\"  placeholder=\"分数\" />\r\n" + 
