...
public class CpplintPlug implements IPlug {
	...
	@Override
	public ResultMsg singleExecute(String configInfo, String filePath, String logDir)
			throws FileNotFoundException, FileNotDirectoryException, FileNotCOrCppExcption {
		// 检查传入的filePath是否存在且不是一个目录
		File taskFile = new File(filePath);
		if (!taskFile.exists() || taskFile.isDirectory()) {
			System.err.println("作业路径为 :" + taskFile);
		...
		if (!logFile.exists()) {
			System.err.println("日志目录为 :" + logDir);
		...
		if (!taskSuffix.equals(".c") && !taskSuffix.equals(".cpp")) {
			System.err.println("待检查的作业为 :" + filePath);
		...
		StringBuilder executeCommand = new StringBuilder(
				"python" + " " + DefaultConfigInfo.toolPath + " " + DefaultConfigInfo.outputFormat + " " + "-o" + " "
		...
		try {
			...
			System.err.println("作业执行失败！执行命令错误，请检查！执行的命令为: " + executeCommand.toString());
			e.printStackTrace();
			return null;
		}
		...
		try {
			...
			System.err.println("日志路径错误，请检查！日志路径为: " + logPath);
			e.printStackTrace();
			return null;
		} catch (IOException e) {
			System.err.println("关闭文件IO出错!");
			e.printStackTrace();
			return null;
		}
	...
	@Override
	public List<ResultMsg> batchExecute(String configInfo, String srcDir, String logDir)
		...
		// 检查该文件目录是否存在
		if (!srcFile.exists()) {
			System.err.println("待检查的作业目录为 :" + srcDir);
			throw new FileNotFoundException("该作业目录不存在！");
		}
		if (!logFile.exists()) {
			System.err.println("日志目录为 :" + logDir);
			throw new FileNotFoundException("该日志目录不存在！");
		}
		// 检查打开的是不是一个文件目录
		if (!srcFile.isDirectory()) {
			System.err.println("待检查的作业路径为 :" + srcDir);
			throw new FileNotDirectoryException("该作业路径不是一个文件目录！");
		}
		if (!logFile.isDirectory()) {
			System.err.println("日志路径为 :" + logDir);
			throw new FileNotDirectoryException("该日志路径不是一个文件目录！");
		}
		// 获取该文件目录下的所有文件名
		...
		for (String item : fileList) {
			...
			ResultMsg ret = null;
			try {
				ret = singleExecute(configInfo, filePath, logDir);
			} catch (FileNotCOrCppExcption e) {
				e.printStackTrace();
	...
	public String getHtml(String preSetting) {
		...
		if (preSetting != null) {
			...
			String scoreOfFuncAnnotation = "'' ", scoreOfFuncName = "'' ", scoreOfFuncParamtersNum = "'' ",
					...
					scoreOfNestedNum = "'' ", scoreOfUseGoto = "'' ", scoreOfLineLength = "'' ", scoreOfIdentationStyle = "'' ",
					scoreOfOperationSpace = "'' ", scoreOfKeyWordsUseBraces = "'' ", scoreOfExtendRules = "'' ";
			// 保存规则
			...
			// 前端页面初始化
			html = "<div id = \"cpplint\" >\r\n" + "		总分\r\n"
					+ "		<input type=\"text\" name=\"totalScore\" value=" + '"' + configObj.getTotalScore().toString() + '"' + " "
					+ "placeholder=\"大于0小于等于100\" >\r\n" + "	    <br />\r\n" + "		\r\n" + "		检查函数注释\r\n"
	...
	public String getChart() {
		String errorType = "";
		String errorNum = "";
		String chart = " <div id=\"container\" style=\"height: 100%\"></div>\r\n" + 
